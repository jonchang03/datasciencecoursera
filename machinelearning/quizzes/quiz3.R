...
# K is small => more bias (larger), but less variance (smaller)
# in leave one out, k is equal to the sample size (leave out one sample each time until you go through rest of training data)
...
# Fit a classification tree where Area is the outcome variable. Then predict the value of area for the following data
# frame using the tree command with all defaults
newdata = as.data.frame(t(colMeans(olive))) # Test Set
...
missTrain <- missClass(trainSA$chd, predict(modelFit, newdata = trainSA))
## 0.2727273
missTest <- missClass(testSA$chd, predictions)
## 0.3116883
